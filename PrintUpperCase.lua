...
local function gotoCheckAccuracy()
	...
	--loadsave.saveTable(twoDTable, "printUpper"..letterArray[letterCount]..".json")
...
local function onObjectTouch( event )
	...
	if ( event.phase == "ended" ) then
		...
		if(endX > boundaryXmin and endY > boundaryYmin and endX < boundaryXmax and endY < boundaryYmax) then	
			savedTable = loadsave.loadTable("printUpper"..letterArray[letterCount]..".json")
...
local numFrames = 4
    local options = {
        ...
        height = 338,
        numFrames = numFrames,
        sheetContentWidth = 1188,
        sheetContentHeight = 338
    }


local imageSheet = graphics.newImageSheet("pngs/upperCasePrint.png", options)

  local sequenceData ={
                       {name = "a", start = 1, count = 1},
                       {name = "b", start = 2, count = 1},
                       {name = "o", start = 3, count = 1},
                       {name = "s", start = 4, count = 1}
  }

  local letterFrames = {"a","b","o","s"}
